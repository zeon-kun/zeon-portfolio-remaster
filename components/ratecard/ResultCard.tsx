...
export function ResultCard({ score, estimate, onRestart, lang }: ResultCardProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Score + tier */}
      <div className="border border-foreground/10 bg-background/40 backdrop-blur-sm p-6 flex flex-col items-center gap-4">
        <p className="text-[9px] font-mono uppercase tracking-[0.25em] text-muted/50 self-start">
          ...
          {top3.map((c, i) => {
            ...
            return (
              <li key={c.id} className="flex items-start gap-2 text-sm">
                <span className="text-[9px] font-mono text-accent-primary tabular-nums flex-shrink-0 mt-0.5">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <span className="text-foreground/70">
                  <span className="font-semibold text-foreground">{name}</span> — {val}
                </span>
              </li>
            );
          })}
      ...
      {/* Rate estimate */}
      <div className="border border-foreground/10 bg-background/40 backdrop-blur-sm p-6 flex flex-col gap-5">
        <p className="text-[10px] font-mono font-bold uppercase tracking-[0.2em] text-muted">
          {isJp ? "費用概算" : "Rate Estimate"}
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xl font-black tabular-nums text-foreground" style={{ fontFamily: "var(--font-mono)" }}>
              {formatIDR(estimate.idrFee.low)} – {formatIDR(estimate.idrFee.high)}
            </p>
            <p className="text-[9px] font-mono text-muted/50">
              {isJp ? "時間単価ベース" : "Hourly rate basis"}: {formatIDR(estimate.idr.low)} – {formatIDR(estimate.idr.high)}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <p className="text-[9px] font-mono uppercase tracking-[0.2em] text-muted/60">
              {isJp ? "プロジェクト費用 (USD)" : "Project Fee (USD)"}
            </p>
            <p className="text-xl font-black tabular-nums text-foreground" style={{ fontFamily: "var(--font-mono)" }}>
        ...
        <div className="border-t border-foreground/8 pt-4 flex flex-col gap-3 md:flex-row md:gap-6">
